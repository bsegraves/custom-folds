...
module.exports = {
	...
	foldTopLevel() {
		let [editor, commentChars] = CustomFolds._getEditor();
			...
			if (editor.isBufferRowCommented(c)) {
				CustomFolds.prefix.forEach((prefix, index) => {
					...
					if (line.replace(commentChars,'').trim().startsWith(prefix)) {
						const endRow = CustomFolds._rowOfEndTag(prefix, postfix, editor, commentChars, c);
						if (c < endRow) {
	...
	foldRow(row) {
		...
		if (editor.isBufferRowCommented(row)) {
			for (let c=0, cLen=CustomFolds.prefix.length; c<cLen; ++c) {
				...
				if (line.replace(commentChars,'').trim().startsWith(prefix)) {
					const endRow = CustomFolds._rowOfEndTag(prefix, postfix, editor, commentChars, row);
					if (row < endRow) {
	...
	_getEditor() {
		...
		return [editor, commentChars];
	},

	// takes nesting into account
	_rowOfEndTag(prefix, postfix, editor, commentChars, startRow) {
		let result = -1;
	...
	_addClickEvent(editor) {
		...
		let gutter = editorView.shadowRoot.querySelector('.gutter');
		$(gutter).on('click', '.line-number.custom-folds-start:not(.folded) .icon-right', function(event) {
			let row = event.target.parentElement.dataset.bufferRow;
